---
name: delete-project
description: Use when the user wants to delete or remove a project from this blog. Triggers on Chinese phrases that contain "项目" (project) such as "删除项目 xxx.html"、"把 xxx 项目删掉"、"移除项目 xxx"、"xxx 项目下掉"、"不要 xxx.html 项目了"，also matches English variants like "delete project xxx.html" or "remove the xxx project". The skill cleans up the HTML source under /projects/ and the registry entry in src/data/projects.js. Do NOT trigger for editing project content, creating new projects, deleting articles (use delete-article), or renaming files.
---
...
1. Deletes `projects/<slug>.html` (the HTML source at project root).
...
- "删除项目 hello-world.html" / "删除项目 hello-world"
- "把 react-tips 这个项目删掉"
- "移除项目 deploy-notes"
- "xxx 项目下掉"、"不要 xxx.html 项目了"
- "delete project xxx.html" / "remove the xxx project"

Strip a trailing `.html` if the user included it; the slug is what matters.

## When NOT to use

- **Editing content** of a project → use the `Edit` tool on the `.html` file directly.
- **Creating a new project** → follow the three-step rule in `CLAUDE.md` (file + import + metadata record).
- **Deleting an article** → use the `delete-article` skill. If the user says "删除 xxx.html" *without* the word "项目", that is ambiguous (articles are still `.md`) — ask before proceeding.
...
- HTML sources live at `projects/<slug>.html` (project root, *not* under `src/`). Drafts that haven't been published live in `projects-draft/` and are not this skill's concern.
...
From the user message, extract the slug. If the user wrote `foo.html`, the slug is `foo`. The expected import variable is the camelCase form (e.g. `react-tips` → `reactTips`). Always confirm the user actually said "项目" / "project" somewhere in the message — if they didn't, this is the wrong skill and the user almost certainly meant an article.

### 2. Locate the three touch points

Search for all three; **abort and report** if any one of them is missing:

- `projects/<slug>.html` on disk (use `ls` or `Read`).
- The import line in `src/data/projects.js` matching `from '../../projects/<slug>.html?raw'`.
...
```
即将删除项目 `<slug>`：
- 文件：projects/<slug>.html
- import：src/data/projects.js 第 N 行（import <var> from '../../projects/<slug>.html?raw'）
...
3. `Bash rm projects/<slug>.html` to delete the source file.
...
- **User said "删除 xxx.html" with no "项目" in the message** → this is suspicious: articles are still `.md`, and projects are now `.html`. A bare `删除 xxx.html` with no project/article word is almost certainly a copy-paste error. Ask once whether they meant an article or a project before proceeding.
